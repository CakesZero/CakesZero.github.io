...
async function genKeys(keyCount) {
  ...
  if (!chat_id.value) return alert('Нужен chat_id!')

  chat_id.disabled = true
  ...
  for (game of games) {
    ...
    const resp = await sendToTg(chat_id.value, text)
...
async function getChatId() {
  ...
  const name = prompt('Введи свое имя в Телеграм')
  ...
  if (name == chat.first_name) document.getElementById('chat-id').value = chat.id
